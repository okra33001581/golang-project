...
func main() {
	...
	rows, err := db.Query("SELECT id,date,company_in,third_in,deposit,common_deposit,benefit,total_rebate,day_salary,bankcard_out,third_out,user_subtraction,artifical_withdraw,total,merchant_id,merchant_name FROM report_finance1")
	checkErr(err)
	...
	for rows.Next() {
		...
		req := elastic.NewBulkIndexRequest().Index("report_finance1").Type("doc").Id(id).Doc(tweet)
